"""
End-to-end evaluation grid for Dots and Boxes.

Sweeps (rows, cols) × oracle_depth × strategies. Mirrors eval_minimax.py.

Forced-pass states (where one player just completed a box and earned the
extra turn) are handled transparently — every P1/P2 baseline returns PASS
on a forced-pass state, and `state.children = {PASS: ...}` so play_one's
fallback to `next(iter(state.children))` always lands on PASS even if a
Mealy emits something else.

Usage:
    python -m src.scripts.eval_dab
    python -m src.scripts.eval_dab --shapes 2,2 --oracle-depths None 0 1 2
"""
...
from src.baselines.dots_and_boxes import (
...
def run_grid(shapes, oracle_depths, n_games,
             lstar_Ks, lstar_depth_ns,
             uct_Ks, ql_episodes,
             lstar_timeout, out_path) -> list:
    rows: list = []

    for (rows_, cols_) in shapes:
        shape_str = f'{rows_}x{cols_}'
        print(f'\n=== DaB {shape_str} ===')

        nfa       = DotsAndBoxesNFA(rows=rows_, cols=cols_)
        root      = nfa.root
        p1_inputs = nfa.p1_alphabet

        print(f'  building OptimalP1 (full minimax solve)...')
        optimal_p1 = OptimalP1(rows=rows_, cols=cols_, seed=0)
        ...
        for name, make_p2 in [('Random',  lambda: RandomP2(seed=0)),
                              ...
                              ('Optimal', lambda r=rows_, c=cols_: OptimalP2(rows=r, cols=c, seed=0))]:
            p2 = make_p2()
            t0 = time.time()
        ...
        for ep in ql_episodes:
            t0 = time.time()
            p2 = P2_QLearn(rows=rows_, cols=cols_, n_episodes=ep, seed=0)
            scores = evaluate(p2, root, p1_makers, n_games)
            rows.append({'shape': shape_str, 'oracle_depth': '*',
        ...
        for od in oracle_depths:
            print(f'\n  [oracle_depth={od}]')
            oracle = DotsAndBoxesOracle(nfa, depth=od)
...
def _shape_arg(v: str):
    a, b = v.split(',')
    return (int(a.strip()), int(b.strip()))


def main():
    parser = argparse.ArgumentParser(description='Eval grid for Dots and Boxes.')
    parser.add_argument('--shapes', nargs='+', type=_shape_arg,
                        default=[(2, 1), (2, 2)],
                        help='Comma-separated rows,cols pairs (e.g. "2,1" "2,2").')
    ...
    parser.add_argument('--out',            type=str, default='outputs/eval_dab.csv')
    args = parser.parse_args()

    out_path = Path(args.out)
    print(f'Eval grid for Dots and Boxes')
    print(f'  shapes={args.shapes}  oracle_depths={args.oracle_depths}  n_games={args.n_games}')
