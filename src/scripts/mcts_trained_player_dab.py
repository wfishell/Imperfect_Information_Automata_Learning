"""
Two-phase MCTS Dots-and-Boxes player (apples-to-apples with L*).

Phase 1 — TRAINING: T UCB1 rollouts from initial state build a search
tree with pairwise oracle.compare() reward, backed up along the path.

Phase 2 — EVALUATION: argmax_a N(s,a) on a frozen tree, random fallback
on out-of-tree states. ZERO oracle calls during eval. PASS / forced_pass
honoured at both phases (random rollouts, lookup, fallback).

Usage:
    python -m src.scripts.mcts_trained_player_dab --rows 2 --cols 2 --train-K 5000
"""
    ...
    for _ in range(T):
        ...
        while True:
            if node is None or node.is_terminal():
                break
            # Forced pass: only one legal action, no UCB choice
            if getattr(node, 'forced_pass', False):
                trace.append(PASS)
                node = node.children[PASS]
                continue
            key = tuple(trace)
            if key not in tree:
                tree[key] = {'visits': 0,
        ...
        while roll_node is not None and not roll_node.is_terminal():
            if getattr(roll_node, 'forced_pass', False):
                a = PASS
            else:
...
def _play_one_game(nfa, tree: dict, rng: random.Random, stats: dict):
    ...
    while not state.is_terminal():
        if state.player == 'P1':
            action = PASS if state.forced_pass else rng.choice(list(state.children.keys()))
        else:
            action = (PASS if state.forced_pass
                      else policy_action(tree, trace,
                                         list(state.children.keys()),
                                         rng, stats))
        trace.append(action)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Two-phase trained MCTS Dots-and-Boxes player.'
    )
    ...
    print(f'Trained-MCTS DaB  {args.rows}x{args.cols}  train_K={args.train_K}  '
