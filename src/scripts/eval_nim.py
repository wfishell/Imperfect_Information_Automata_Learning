"""
End-to-end evaluation grid for Nim.

Sweeps piles × oracle_depth × strategies. Mirrors eval_minimax.py.

Usage:
    python -m src.scripts.eval_nim
    python -m src.scripts.eval_nim --piles "1,2,3" "1,3,5" --oracle-depths None 2
"""
...
def print_summary(rows: list) -> None:
    if not rows:
        return
    print()
    print(f'{"shape":>14}  {"od":>4}  {"strategy":<25}  {"params":<22}  '
    ...
    for r in rows:
        ...
        print(f'{r["shape"]:>14}  {str(od):>4}  {r["strategy"]:<25}  {r["params"]:<22}  '
...
def run_grid(piles_list, oracle_depths, n_games,
    ...
    for piles in piles_list:
        shape_str = ','.join(str(p) for p in piles)
        print(f'\n=== Nim piles=({shape_str}) ===')
        ...
        optimal_p1 = OptimalP1(piles=piles, seed=0)
        ...
        for name, make_p2 in [('Random',  lambda: RandomP2(seed=0)),
                              ...
                              ('Optimal', lambda p=piles: OptimalP2(piles=p, seed=0))]:
            p2 = make_p2()
            t0 = time.time()
        ...
        for ep in ql_episodes:
            t0 = time.time()
            p2 = P2_QLearn(piles=piles, n_episodes=ep, seed=0)
            scores = evaluate(p2, root, p1_makers, n_games)
            rows.append({'shape': shape_str, 'oracle_depth': '*',
...
def _piles_arg(v: str):
    return tuple(int(x) for x in v.replace('(', '').replace(')', '').split(','))


def main():
    parser = argparse.ArgumentParser(description='Eval grid for Nim.')
    parser.add_argument('--piles', nargs='+', type=_piles_arg,
                        default=[(1, 2, 3), (1, 3, 5), (2, 3, 4), (1, 2, 3, 4)])
    ...
    parser.add_argument('--lstar-depth-ns', nargs='+', type=int, default=[ 2, 4])
    ...
    parser.add_argument('--out',            type=str, default='outputs/eval_nim.csv')
    args = parser.parse_args()

    out_path = Path(args.out)
    print(f'Eval grid for Nim')
    print(f'  piles={args.piles}  oracle_depths={args.oracle_depths}  n_games={args.n_games}')
