"""
Pure-MCTS Dots and Boxes baseline (no automaton learning).
...
Usage:
    python -m src.scripts.mcts_player_dab --rows 2 --cols 2 --K 20 --games 200
"""
...
def _random_rollout(nfa: DotsAndBoxesNFA, trace: list, rng: random.Random) -> list:
...
def _mcts_p2_action(nfa: DotsAndBoxesNFA, oracle, trace: list, K: int,
                    c: float, rng: random.Random) -> int | str:
    """
...
def _play_one_game(nfa: DotsAndBoxesNFA, oracle, K: int, c: float,
    ...
    while not state.is_terminal():
        if state.player == 'P1':
            ...
            action = PASS if state.forced_pass else _mcts_p2_action(nfa, oracle, trace, K, c, rng)
        trace.append(action)
        state = state.children[action]
    return state.winner()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Pure-MCTS Dots and Boxes baseline (no automaton learning).'
    )
    ...
    print(f'UCB1-MCTS player  {args.rows}x{args.cols}  K={args.K}  c={args.c}  '
