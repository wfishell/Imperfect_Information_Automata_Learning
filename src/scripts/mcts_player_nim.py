"""
Pure-MCTS Nim baseline (no automaton learning).
...
Usage:
    python -m src.scripts.mcts_player_nim --piles 1 2 3 --K 20 --games 200
"""
...
def _random_rollout(nfa: NimNFA, trace: list, rng: random.Random) -> list:
...
def _mcts_p2_action(nfa: NimNFA, oracle, trace: list, K: int,
...
def _play_one_game(nfa: NimNFA, oracle, K: int, c: float,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Pure-MCTS Nim baseline (no automaton learning).'
    )
    parser.add_argument('--piles',        type=int, nargs='+', default=[1, 2, 3],
                        help='Initial pile sizes (default: 1 2 3)')
    ...
    print(f'UCB1-MCTS Nim  piles={tuple(args.piles)}  K={args.K}  c={args.c}  '
