"""
Aggregate outputs/eval_dab.csv across (shape, oracle_depth).
...
Usage:
    python -m src.scripts.aggregate_dab
    python -m src.scripts.aggregate_dab --keep-shape
"""
...
CSV_IN  = Path(__file__).parents[2] / 'outputs' / 'eval_dab.csv'
CSV_OUT = Path(__file__).parents[2] / 'outputs' / 'eval_dab_aggregated.csv'
...
def main() -> None:
    ...
    write_and_print(df, CSV_OUT, 'eval_dab aggregated')
