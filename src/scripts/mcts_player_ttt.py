"""
Pure-MCTS Tic-Tac-Toe baseline (no automaton learning).
...
Usage:
    python -m src.scripts.mcts_player_ttt --K 20 --games 200
"""
...
def _random_rollout(nfa: TicTacToeNFA, trace: list, rng: random.Random) -> list:
...
def _mcts_p2_action(nfa: TicTacToeNFA, oracle, trace: list, K: int,
                    c: float, rng: random.Random) -> int:
    """
    Allocate K rollouts across legal P2 actions via UCB1, with a
    pairwise-tournament reward.

    Seeding phase: one rollout per arm, then every pair of seed
    rollouts is compared once via oracle.compare(). Each arm's
    visits[a] / wins[a] track total appearances and total preferences.

    UCB phase: each subsequent pull picks the arm with the highest
    ucb1 = wins/visits + c·√(ln Σvisits / visits), does one rollout,
    then compares it against a random stored rollout from another arm.
    visits/wins are updated symmetrically for both sides of the
    compare. Choice = arm with the most visits at the end.
    """
    state = nfa.get_node(trace)
...
def _play_one_game(nfa: TicTacToeNFA, oracle, K: int, c: float,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Pure-MCTS Tic-Tac-Toe baseline (no automaton learning).'
    )
    ...
    print(f'UCB1-MCTS Tic-Tac-Toe  K={args.K}  c={args.c}  games={args.games}  '
